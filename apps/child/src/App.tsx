...
const App: React.FC = () => {
  ...
  // Puzzle State
  const [puzzleState, setPuzzleState] = useState<'idle' | 'locked' | 'running'>('idle');
  ...
  const handleVerifyPuzzlePassword = () => {
    ...
    if (puzzleInput === eventPass) {
      ...
      setPuzzleInput('');
      setPuzzleError(false);
      emit('CONNECTION_MSG', { text: 'PUZZLE_SOLVED: Correct unlock password entered.' });
    } else {
      setPuzzleError(true);
    }
  };
      ...
      <AnimatePresence>
          {puzzleState === 'locked' && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="fixed inset-0 z-[500] bg-[#09090b] backdrop-blur-3xl flex flex-col items-center justify-center p-6 text-center"
              >
                  {/* Digital aesthetic details */}
                  ...
                  <motion.div
                    ...
                    animate={{ scale: 1, y: 0 }}
                    className="w-full max-w-md glass-panel p-10 rounded-[32px] border-white/5 bg-white/0 relative z-10 flex flex-col items-center shadow-[0_32px_64px_-12px_rgba(0,0,0,0.8)]"
                  >
                      <div className="w-16 h-16 bg-red-500/10 rounded-[24px] flex items-center justify-center mb-8 border border-red-500/20 animate-pulse">
                          <Lock className="text-red-400" size={32} />
                      </div>

                      <h2 className="text-2xl font-black tracking-[0.3em] uppercase text-white mb-2">SYSTEM_RESTRICTED</h2>
                      <p className="text-xs text-white/40 uppercase tracking-[0.1em] mb-10 max-w-xs leading-relaxed">
                      ...
                      <div className="relative w-full mb-2">
                          <input
                              ...
                              className={`w-full bg-white/5 border rounded-2xl px-6 py-4 text-center outline-none focus:border-white/20 transition-all text-xl font-mono tracking-[0.5em] ${puzzleError ? 'border-red-500' : 'border-white/10'}`}
                              value={puzzleInput}
                              onChange={(e) => {
                      ...
                      <button
                          ...
                          className="w-full py-4 rounded-2xl bg-white text-black font-black text-xs uppercase tracking-[0.2em] hover:bg-white/90 transition-all flex items-center justify-center gap-2"
                      >
                          解除システム実行
                      </button>
                  </motion.div>
